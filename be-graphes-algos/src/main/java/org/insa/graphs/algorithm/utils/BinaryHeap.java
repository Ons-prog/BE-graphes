...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    public void remove(E x) throws ElementNotFoundException {
          ...
          // 1) trouver l’indice de x
        int index = array.indexOf(x);
        if (this.isEmpty() || index == -1 || index >= currentSize) {
            throw new ElementNotFoundException(x);
        }


        // 2) récupérer la dernière valeur
        int lastIndex = currentSize - 1;
        E last = array.get(lastIndex);

        // 3) si ce n’était pas déjà le dernier, on la place à la position 'index'
        if (index != lastIndex) {
            arraySet(index, last);
        }

        // 4) on décrémente currentSize 
        currentSize--;
        

        // 5) rééquilibrage si on a remplacé un élément ailleurs que dans la queue
        if (index != lastIndex) {
            percolateDown(index);
            percolateUp(index);
        }
